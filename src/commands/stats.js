...
class Stats {
    execute(params, message) {
        ...
        if (params.length > 0) {
            ...
            if (params.length === 1) {
                this.sendGeneralStats(playerName, message);
            } else {
                // Check validity of each parameter
                if (!isNaN(params[1])) {
                    if (!isNaN(params[2])) {
                        if (teams[`${params[3].toLowerCase()}`] !== undefined) {
                            this.sendMatchStats(playerName, params[1], params[2], params[3].toLowerCase(), message);
                        } else {
    ...
    sendGeneralStats(playerName, message) {
        request.get({
            ...
            let playerIndex = this.findPlayerIndex(body, playerName);
            ...
            let timePlayed = this.convertSeconds(body['data'][`${playerIndex}`]['time_played_total']);
            let msg;
    ...
    sendMatchStats(playerName, stage, week, opponent, message) {    
        let playerId;
        let playerTeam;
        let name;
        let competitorOne;
        ...
        this.getRequest('https://api.overwatchleague.com/stats/players').then((playerBody) => {
            // Get the player's id and their team's id
            let playerIndex = this.findPlayerIndex(playerBody, playerName);
            if (playerIndex >= 0) {
            ...
            return this.getRequest('https://api.overwatchleague.com/schedule')
        }).then((scheduleBody) => {
            ...
            async.map(urls, (url, callback) => {
                ...
                let timePlayed = this.convertSeconds(stats[4] / 1000);
                let msg = 'Player: **' + name +
    ...
    findPlayerIndex(body, playerName) {
